...
export default function App() {
  ...
  const [equipment, setEquipment] = useState(() => {
    const saved = localStorage.getItem("hk_equipment");
    return saved ? JSON.parse(saved) : initialEquipment;
  });
  const [selectedEquip, setSelectedEquip] = useState(null);
  const [view, setView] = useState("dashboard");
  const [history, setHistory] = useState(() => {
    const saved = localStorage.getItem("hk_history");
  ...
  const [reminders, setReminders] = useState(() => {
    const saved = localStorage.getItem("hk_reminders");
  ...
  // Property To-Do
  const [propertyTodos, setPropertyTodos] = useState(() => {
    const saved = localStorage.getItem("hk_property");
    return saved ? JSON.parse(saved) : initialPropertyTodos;
  });
